...
class GetAssetPages implements UseCase<any, Response> {
  ...
  public async execute(req, res): Promise<any> {
    ...
    const { count, rows }: any =
      await this._assetRepo.findPages(objQuery);
    const assets = rows.length > 0 ? rows : [];
    ...
    console.log(`END << Asset Page ${JSON.stringify(result)}`);
    return right(Result.ok(result));
  }
}

export { GetAssetPages };
